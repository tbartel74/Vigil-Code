...
description: ClickHouse analytics and Grafana dashboard configuration for Vigil Guard monitoring. Use when querying logs, creating dashboards, analyzing threat detection metrics, optimizing schema, investigating events, working with n8n_logs database, managing retention policies, or using Map type for score_breakdown.
version: 1.6.11
...
## Map Type Usage (ClickHouse 24.1+)

### score_breakdown Field

The `score_breakdown` field uses ClickHouse's **Map** type for efficient category→score storage.

**Schema:**
```sql
score_breakdown Map(String, Float64)  -- Category name → Score value
```

### Querying Map Fields

**1. Get all categories that contributed to a decision:**
```sql
SELECT
  original_input,
  final_status,
  mapKeys(score_breakdown) AS categories,
  mapValues(score_breakdown) AS scores
FROM n8n_logs.events_processed
...
**2. Find events with specific category:**
```sql
...
**3. Get score for specific category:**
```sql
SELECT
  original_input,
...
**4. Filter by category score threshold:**
```sql
SELECT * FROM n8n_logs.events_processed
WHERE score_breakdown['SQL_XSS_ATTACKS'] > 50
ORDER BY timestamp DESC;
```

**5. Aggregate category statistics:**
```sql
-- Count events per category (requires arrayJoin on mapKeys)
SELECT
  category,
  count() AS occurrences,
...
FROM (
  SELECT
    ...
    score_breakdown[category] AS score
  FROM n8n_logs.events_processed
...
**6. Top contributing categories per event:**
```sql
-- Get top 3 categories by score for each event
SELECT
  original_input,
  final_status,
  arraySort((x, y) -> y, mapKeys(score_breakdown), mapValues(score_breakdown)) AS sorted_categories,
  arraySort((x, y) -> -y, mapKeys(score_breakdown), mapValues(score_breakdown)) AS sorted_scores
FROM n8n_logs.events_processed
WHERE final_status = 'BLOCKED'
LIMIT 10;
```

### Map vs Array of Tuples

**Advantages of Map type:**
- Direct key access: `score_breakdown['SQL_XSS_ATTACKS']` (no array scanning)
- Efficient filtering: `mapContains(score_breakdown, 'key')`
- Built-in functions: `mapKeys()`, `mapValues()`, `mapApply()`
- Better compression for sparse data

**Example Comparison:**
```sql
-- OLD: Array(Tuple(String, Float64))
SELECT * FROM events WHERE arrayExists(x -> x.1 = 'SQL_XSS_ATTACKS', score_breakdown);

-- NEW: Map(String, Float64)
SELECT * FROM events WHERE mapContains(score_breakdown, 'SQL_XSS_ATTACKS');
```
